module core_cpu(
    ...
    wire            bcb_clear_en;
    wire            bcb_lsu_busy;
    wire            update_branch_en_ex;
    wire            update_branch_not_en_ex;
    wire            bcb_update_branch_en;
    wire            bcb_update_branch_not_en;
    BCB u_bcb(
        .bcu_done           (bcu_done),
        ...
        .update_branch_en_ex    (update_branch_en_ex),
        .update_branch_not_en_ex    (update_branch_not_en_ex),
        .bcb_update_branch_en   (bcb_update_branch_en),
        .bcb_update_branch_not_en   (bcb_update_branch_not_en)
    );
    ...
    BTB u_btb(
        ...
        .update_branch_en   (bcb_update_branch_en),
        .update_branch_not_en   (bcb_update_branch_not_en)
    );
    ...
    reg     [31:0]  regsrc1_id;
    reg     [31:0]  regsrc2_id;
    wire    src1_bypass_en, src1_bypass_en_stall, ccb_src1_en, ccb_src1_en_stall;
    wire    src2_bypass_en, src2_bypass_en_stall, ccb_src2_en, ccb_src2_en_stall;
    assign src1_bypass_en = cdb_en && (rs1_addr == cdb_id) && SAT[rs1_addr];
    assign src2_bypass_en = cdb_en && (rs2_addr == cdb_id) && SAT[rs2_addr];
    assign src1_bypass_en_stall = cdb_en && (rs1_addr_id == cdb_id) && SAT[rs1_addr_id];
    assign src2_bypass_en_stall = cdb_en && (rs2_addr_id == cdb_id) && SAT[rs2_addr_id];

    assign ccb_src1_en = CCB_en && (rs1_addr == CCB_id) && SAT[rs1_addr];   
    assign ccb_src2_en = CCB_en && (rs2_addr == CCB_id) && SAT[rs2_addr];
    assign ccb_src1_en_stall = CCB_en && (rs1_addr_id == CCB_id) && SAT[rs1_addr_id];
    assign ccb_src2_en_stall = CCB_en && (rs2_addr_id == CCB_id) && SAT[rs2_addr_id];
    wire    [4:0]   CCB_id;
    wire            CCB_en;
    wire    [31:0]  CCB_data;
    reg     src1_bypass_en_id, src2_bypass_en_id;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            pc_id <= 32'b0;
            pred_hit_id <= 1'b0;
            ...
            regsrc1_id <= 32'b0;
            regsrc2_id <= 32'b0;
            src1_bypass_en_id <= 1'b0;
            src2_bypass_en_id <= 1'b0;
        end else begin
            if (!stall || flush) begin
                pc_id <= pc_if;
                ...
                imm_id <= imm;
                op_id <= op;
                regsrc1_id <= ccb_src1_en ? CCB_data:
                                src1_bypass_en ? cdb_result: regfiles[rs1_addr];
                regsrc2_id <= ccb_src2_en ? CCB_data:
                                src2_bypass_en ? cdb_result: regfiles[rs2_addr];
                src1_bypass_en_id <= ccb_src1_en || src1_bypass_en;
                src2_bypass_en_id <= ccb_src2_en || src2_bypass_en;
            end else begin
                regsrc1_id <= ccb_src1_en_stall ? CCB_data:
                                src1_bypass_en_stall ? cdb_result: regfiles[rs1_addr_id];
                regsrc2_id <= ccb_src2_en_stall ? CCB_data:  
                                src2_bypass_en_stall ? cdb_result: regfiles[rs2_addr_id];
                src1_bypass_en_id <= ccb_src1_en_stall || src1_bypass_en_stall;
                src2_bypass_en_id <= ccb_src2_en_stall || src2_bypass_en_stall;
            end 
        end
    end
    ...
    assign regsrc1 = regsrc1_id;
    assign regsrc2 = regsrc2_id;
    ...
    Issue u_issue(
        ...
        .cdb_result     (cdb_result),
        .cdb_id         (cdb_id),
        .src1_bypass_en_id (src1_bypass_en_id),
        .src2_bypass_en_id (src2_bypass_en_id)
        // .CCB_en         (CCB_en_id),
        // .CCB_id         (CCB_id_id),
        // .CCB_data       (CCB_data_id)
    );
    ...
    EX u_EX(
        ...
        .bcu_done       (bcu_done),
        .update_branch_en (update_branch_en_ex),
        .update_branch_not_en (update_branch_not_en_ex),
        .wb_id          (wb_id_ex),
        .wb_en          (wb_en_ex),
