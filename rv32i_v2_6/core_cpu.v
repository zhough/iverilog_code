...
module core_cpu (
    ...
    reg pred_hit_id, pred_hit_ex;
    reg [31:0]  pred_target_id, pred_target_ex;
    always @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            ...
            update_pc <= 32'b0;
            update_target <= 32'b0;
            update_clear_en <= 1'b0;
        end else begin
            update_en <= ((jump_taken_ex || branch_taken_ex) && (~pred_correct)) || branch_not_taken_hit;
            update_pc <= pc_ex;
            update_target <= branch_not_taken_hit ? pc_ex + 4:
                             jump_taken_ex ? jump_target_ex :
                             branch_taken_ex ? branch_target_ex : 32'b0;
            update_clear_en <= branch_not_taken_hit;
            pred_hit_ex <= pred_hit_id;
            pred_target_ex <= pred_target_id;
            if (id_nop) begin
                pred_hit_id <= pred_hit_id;
                pred_target_id <= pred_target_id;
            end else begin  
                pred_hit_id <= pred_hit;
                pred_target_id <= pred_target;
            end
        end
    end

    wire target_correct = (jump_taken_ex && (jump_target_ex == pred_target_ex)) || (branch_taken_ex && (branch_target_ex == pred_target_ex));
    // 更新的触发是实际要跳转但是没有hit或者预测错了
    // assign update_en = ((jump_taken_ex || branch_taken_ex) && (~pred_correct)) || branch_not_taken_hit;
    // assign update_pc = pc_ex;
    // assign update_target =  branch_not_taken_hit ? pc_ex + 4:
    //                         jump_taken_ex ? jump_target_ex :
    //                         branch_taken_ex ? branch_target_ex : 32'b0;
    assign pred_correct = target_correct && pred_hit_ex;
    // 分支不跳转但是预测跳转了
    wire branch_not_taken_hit = pred_hit_ex && (~(jump_taken_ex || branch_taken_ex)) && (~ins_invalid_ex);
    ...
    always @(negedge rst_n or posedge clk) begin
        if (!rst_n) begin
            ...
            //加入旁路
            a_ex <= RAW1_ex     ? bypass_ex     :
                    RAW1_mem    ? bypass_mem    :
                    RAW1_mem2   ? bypass_mem2   :
                    RAW1_mem3   ? bypass_mem3   :
                    rs[rs1_id];
            b_ex <= RAW2_ex     ? bypass_ex     :
                    RAW2_mem    ? bypass_mem    :
    ...
    reg     ins_invalid_if2;
    always @(*) begin
        jump_taken_if = jump_taken_ex & (~ins_invalid_ex) & (~pred_correct);
        jump_target_if = jump_target_ex;
        branch_taken_if = (branch_taken_ex & (~ins_invalid_ex) & (~pred_correct)) || branch_not_taken_hit;
        branch_target_if = branch_not_taken_hit ? pc_ex + 4 : branch_target_ex;
        ins_invalid_if2 = jump_taken_if || branch_taken_if;
        ins_invalid_id = (jump_taken_if || branch_taken_if) | RAL | RAL2 | RAL3;
        id_nop = RAL | RAL2 | RAL3;
        nop_if = RAL | RAL2 | RAL3;
        ex_stall = 1'b0;
        mem_clear = 1'b0;
    end
    ...
    reg                 hit_mem2;
    reg                 hit_mem3;
    assign  RAW1_ex = (rd_ex == rs1_id) & rd_en_ex & (~is_load_ex) & (rs1_id != 5'b0) & (~ins_invalid_ex);
    assign  RAW2_ex = (rd_ex == rs2_id) & rd_en_ex & (~is_load_ex) & (rs2_id != 5'b0) & (~ins_invalid_ex);
    ...
    assign  RAL =  ((rd_ex == rs1_id & rs1_id != 5'b0) | (rd_ex == rs2_id & rs2_id != 5'b0 & rs2_en_id)) & is_load_ex & (~ins_invalid_ex) & (~ins_invalid_id2);
    assign  RAL2 = ((rd_mem == rs1_id & rs1_id != 5'b0) | (rd_mem == rs2_id & rs2_id != 5'b0 & rs2_en_id)) & is_load_mem & (~hit_mem) & (~ins_invalid_id2);
    assign  RAL3 = ((rd_mem2 == rs1_id & rs1_id != 5'b0) | (rd_mem2 == rs2_id & rs2_id != 5'b0 & rs2_en_id)) & is_load_mem2 & (~hit_mem2) & (~ins_invalid_id2);
