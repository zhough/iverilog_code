module core_cpu(
    ...
    reg     [31:0]  regsrc1_id;
    reg     [31:0]  regsrc2_id;
    wire    src1_bypass_en, src1_bypass_en_stall;
    wire    src2_bypass_en, src2_bypass_en_stall;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (!stall || flush) begin
                ...
                imm_id <= imm;
                op_id <= op;
                regsrc1_id <= src1_bypass_en ? cdb_result: regfiles[rs1_addr];
                regsrc2_id <= src2_bypass_en ? cdb_result: regfiles[rs2_addr];
            end else begin
                regsrc1_id <= src1_bypass_en_stall ? cdb_result: regfiles[rs1_addr_id];
                regsrc2_id <= src2_bypass_en_stall ? cdb_result: regfiles[rs2_addr_id];
            end
        end
    end
