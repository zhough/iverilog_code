module BCU(
    ...
    output  reg             update_branch_en,
    output  reg             update_branch_not_en
...
//对于跳转语句，广播给快照队列，IF，ID同时写buffer
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            update_en <= 1'b0;
            btb_clear_en <= 1'b0;
            update_branch_en <= 1'b0;
            update_branch_not_en <= 1'b0;
        end else begin
            bcu_done <= bcu_en;
            ...
            update_branch_en <= bcu_en && branch_en;
            update_branch_not_en <= bcu_en && !branch_en;
        end
    end
