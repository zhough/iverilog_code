...
module core_cpu (
    ...
    wire    jump_taken_id, branch_taken_id;
    wire    [31:0]  jump_target_id, branch_target_id;
    wire    [4:0]   rs1_id2, rs2_id2;
    ID2 u_ID2(
        .ins        (ins_id),
        .rd_ex      (rd_ex),
        .rd_mem     (rd_mem),
        ...
        .rd_en_mem3 (rd_en_mem3),
        .is_load_ex     (is_load_ex),
        .is_load_mem    (is_load_mem),
        .is_load_mem2   (is_load_mem2),
        .hit_mem        (hit_mem),
        .hit_mem2       (hit_mem2),
        .rs1            (rs[rs1_id2]),
        .rs2            (rs[rs2_id2]),
        ...
        .bypass_mem2    (bypass_mem2),
        .bypass_mem3    (bypass_mem3),
        .pc             (pc_id),
        .jump_taken     (jump_taken_id),
        ...
        if (!rst_n) begin
            ...
            if (ex_clear) begin
                ...
                b_ex <= RAW2_ex     ? bypass_ex     :
                        ...
                        rs[rs2_id];
                // a_ex <= RAW1_mem    ? bypass_mem    :
                //         RAW1_mem2   ? bypass_mem2   :
                //         RAW1_mem3   ? bypass_mem3   :
                //         rs[rs1_id];
                // b_ex <= RAW2_mem    ? bypass_mem    :
                //         RAW2_mem2   ? bypass_mem2   :
                //         RAW2_mem3   ? bypass_mem3   :
                //         rs[rs2_id];
    ...
    always @(*) begin
        ...
        //ex_clear = jump_taken_ex | branch_taken_ex | LAR | LAR2 | LAR3;
        jump_taken_if = jump_taken_id;
        jump_target_if = jump_target_id;
        branch_taken_if = branch_taken_id;
        branch_target_if = branch_target_id;
        id_clear = jump_taken_id | branch_taken_id;
        ex_clear = LAR | LAR2 | LAR3;
    ...
    assign  LAR2 = ((rd_mem == rs1_id & rs1_id != 5'b0) | (rd_mem == rs2_id & rs2_id != 5'b0 & rs2_en_id)) & is_load_mem & (~hit_mem);
    assign  LAR3 = ((rd_mem2 == rs1_id & rs1_id != 5'b0) | (rd_mem2 == rs2_id & rs2_id != 5'b0 & rs2_en_id)) & is_load_mem2 & (~hit_mem2);
    ...
    wire                is_SAW; //write after store
    wire                is_SAW2;
    wire                is_SAW3;
    assign is_SAW = (rs2_ex == rd_mem) & (is_store_ex) & rd_en_mem & (rs2_ex != 5'b0);
    assign is_SAW2 = (rs2_ex == rd_mem2) & (is_store_ex) & rd_en_mem2 & (rs2_ex != 5'b0);
    assign is_SAW3 = (rs2_ex == rd_mem3) & (is_store_ex) & rd_en_mem3 & (rs2_ex != 5'b0);
    assign  store_data_ex  = is_SAW  ? bypass_mem  : 
                             is_SAW2 ? bypass_mem2 : 
                             is_SAW3 ? bypass_mem3 : rs[rs2_ex];
